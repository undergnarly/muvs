...
const phases = [
  ...
  },
  {
    ...
    tasks: [
      ...
      ["Автоматическое сведение и нормализация", 4],
      ["Сборка длинных видео, Shorts и обложек", 6],
      ["Контент-календарь и очередь публикаций", 4],
      ["Контроль качества и тестирование", 4],
    ],
  },
  {
    id: "03",
    ...
    tasks: [
      ...
      ["Генерация и проверка персонального сценария", 8],
      ["TTS, имя, темп и выбор голоса", 6],
      ["Музыка, микширование и экспорт MP3", 8],
      ["Фоновая очередь и статусы генерации", 5],
      ["Хранилище, доставка и удаление файлов", 4],
      ["Админ-панель, мониторинг и QA", 6],
    ],
  },
  {
    id: "04",
    ...
    tasks: [
      ...
      ["Подписка и рекуррентные платежи", 5],
      ["Промокоды и партнерские ссылки", 2],
      ["Воронка, события и продуктовая аналитика", 4],
    ],
  },
  {
    id: "05",
    ...
    tasks: [
      ...
      ["Meditation of the Month", 3],
      ["Email и push-напоминания", 4],
      ["Программы на 7, 21 и 30 дней", 5],
      ["Рекомендации на основе правил", 3],
    ],
  },
  {
    id: "06",
    ...
    tasks: [
      ...
      ["Проверка голосов, имен и произношения", 4],
      ["Мультиязычный SEO и YouTube-процесс", 4],
      ["Языковые safety-фильтры", 4],
    ],
  },
  {
    id: "07",
    ...
    tasks: [
      ...
      ["Плеер, background audio и offline", 12],
      ["Push, календарь и ежедневная практика", 10],
      ["Мобильные покупки и подписки", 8],
      ["QA, публикация App Store и Google Play", 10],
    ],
  },
  {
    id: "08",
    ...
    tasks: [
      ...
      ["Брендированные библиотеки и лицензии", 14],
      ["Партнерский кабинет и API", 12],
      ["Адаптация по настроению и времени суток", 14],
      ["Персональные программы и вариативность", 18],
      ["Рекомендательная система", 16],
      ["Безопасность, нагрузочное тестирование и QA", 10],
    ],
  },
];

const HOURLY_RATE = 30;
const BASELINE_HOURS = 462;
const PROTOTYPE_HOURS = 40;
const estimate = (hours) => hours;
const totalHours = phases.reduce((sum, phase) => sum + phase.hours, 0);
const mvpHours = phases
  .slice(0, 5)
  .reduce((sum, phase) => sum + phase.hours, 0);
const savedHours = BASELINE_HOURS - totalHours;
const savingsPercent = Math.round((savedHours / BASELINE_HOURS) * 100);
...
function Roadmap() {
  ...
  return (
    <main className="med-project">
      ...
      <section className="med-reuse">
        <div>
          <p className="med-kicker">ПЕРЕИСПОЛЬЗОВАНИЕ КОДА</p>
          <h2>На {savedHours} часов меньше</h2>
          <p>
            Existing code сокращает оценку с {BASELINE_HOURS} до {totalHours}{" "}
            часов — экономия {savingsPercent}% без сокращения roadmap.
          </p>
        </div>
        <div className="med-reuse-columns">
          <article>
            <small>ПЕРЕИСПОЛЬЗУЕМ</small>
            <p>
              Формы и order flow, Supabase, storage, фоновые задачи,
              delivery/email, workspace, i18n и admin-инфраструктуру.
            </p>
          </article>
          <article>
            <small>РАЗРАБАТЫВАЕМ</small>
            <p>
              Meditation prompts и safety, TTS, аудиомикширование и
              специализированный UX медитаций.
            </p>
          </article>
        </div>
      </section>
